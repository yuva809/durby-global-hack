  ...
  if (mounted && remaining.done) {
    return (
      <div className={cn("mkt-glass inline-flex items-center gap-2.5 rounded-full px-5 py-2.5", className)}>
        ...
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-hack-signal">Live now</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("flex items-stretch gap-2 sm:gap-3", className)} role="timer" aria-live="off" suppressHydrationWarning>
      {UNITS.map((unit) => (
        <div
          key={unit.key}
          className="relative flex w-16 flex-col items-center overflow-hidden rounded-xl border border-durby-line bg-durby-card py-3 sm:w-20 sm:py-4"
        >
          <span className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" />
          <span className="font-mono text-2xl font-semibold tabular-nums text-durby-ink sm:text-3xl lg:text-4xl" suppressHydrationWarning>
            {String(remaining[unit.key]).padStart(2, "0")}
          </span>
          <span className="mt-1.5 text-[9px] font-semibold uppercase tracking-[0.18em] text-durby-ink-muted sm:text-[10px]">
            {unit.label}
          </span>
        </div>
      ))}
